...
const ThemeSelector = ({ themes, currentTheme, onSelect }) => {
      ...
      >
        ...
        >
          ...
          <div className="flex gap-1.5 mb-2 items-center w-full justify-center">
            {/* Left Syringe (IM) */}
            <div
              className="w-1.5 h-10 rounded-full border border-opacity-30"
              style={{ background: imColor, borderColor: border }}
            />

            {/* AH Card */}
            <div
              className="w-14 h-16 rounded-md border border-opacity-30 flex flex-col p-1"
              style={{
            ...
            >
              <div
                className="h-2.5 w-full rounded-sm mb-1"
                style={{ background: theme.accentAH }}
              />
              <div
                className="h-0.5 w-8/12 rounded-full mb-0.5"
                style={{ background: theme.textPrimary }}
              />
              <div
                className="h-0.5 w-5/12 rounded-full opacity-60"
                style={{ background: theme.textSecondary }}
              />
              <div
                className="mt-auto h-3 w-full rounded-sm"
          ...
          {/* Timeline Preview */}
          <div
            className="w-32 h-12 rounded-lg border border-opacity-30 relative flex justify-center overflow-hidden"
            style={{
          ...
          >
            ...
            />
            <div
              className="absolute right-[55%] top-3 w-6 h-3 rounded-sm"
              style={{ background: theme.accentAH }}
            />
            <div
              className="absolute left-[55%] top-7 w-6 h-3 rounded-sm"
              style={{ background: theme.accentEI }}
            />
          </div>
        </div>
        ...
        <div className="mt-2 px-1 flex items-center justify-between w-full">
          <span
            className="text-[10px] font-black uppercase tracking-wider truncate max-w-[80%]"
            style={{
              color: isSelected
                ? "var(--success-color)"
                : "var(--text-primary)",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h5 className="text-xs font-black text-[var(--text-secondary)] uppercase tracking-wider mb-3 ml-1">
          Світлі теми
        </h5>
