...
const MedTrackerCard = ({
  ...
  // Syringe column
  const syringeColumn = (
    <div className="relative overflow-hidden" style={{ width: '90px', minWidth: '90px' }}>
      {/* Inner shadow overlay to create 3D recessed "well" effect */}
      <div
        className="absolute inset-0 z-10 pointer-events-none"
        style={{
          boxShadow: isAH
            ? 'inset -20px 0 30px -10px rgba(0,0,0,0.4), inset 0 10px 20px -5px rgba(0,0,0,0.3), inset 0 -10px 20px -5px rgba(0,0,0,0.3)'
            : 'inset 20px 0 30px -10px rgba(0,0,0,0.4), inset 0 10px 20px -5px rgba(0,0,0,0.3), inset 0 -10px 20px -5px rgba(0,0,0,0.3)',
        }}
      />
      {/* Syringe - positioned so needle sticks out top but clipped, rod goes out bottom but clipped */}
      <div className="absolute inset-0" style={{ top: '-30px', bottom: '-60px' }}>
        <SyringeSlider
          value={currentDosage}
  ...
  // Main content column
  const contentColumn = (
    <div className="flex flex-col flex-1 min-w-0 p-4 sm:p-5 relative">
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex rounded-lg overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
            {['mg', 'ml'].map((u) => (
              <button
                key={u}
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="relative">
          ...
          <span
            className="absolute -top-2 -right-7 text-sm font-bold"
        ...
        <div className="flex gap-3 mt-4">
          <button
            ...
            className="w-10 h-10 rounded-xl flex items-center justify-center text-xl font-black transition-all hover:scale-110 active:scale-90"
            style={{
      ...
      <div className="mt-auto pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider mb-3 transition-all"
          style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: '0 20px 50px var(--shadow-color-strong), inset 0 1px 0 rgba(255,255,255,0.06)',
      }}
    >
      {/* Subtle top shine */}
      <div
      ...
      <div className={`flex h-full ${isAH ? 'flex-row' : 'flex-row-reverse'}`}>
        {contentColumn}
        {syringeColumn}
      </div>
    </div>
